...
cat(sprintf('Title: %s\nInvestigators: %s (contact), %s, %s,  %s,  %s\nStudy period:%s -- %s\nSite: %s [%s x %s at %s meters]\nAbstract: %s\nMethod: %s\nAssociated publication: %s\n', bulkDensity_meta[6,3], 
            bulkDensity_meta[7,3], bulkDensity_meta[7,4], bulkDensity_meta[7,5], bulkDensity_meta[8,3], bulkDensity_meta[8,4],
            bulkDensity_meta[10,3],bulkDensity_meta[11,3],
            bulkDensity_meta[12,3], bulkDensity_meta[13,3], bulkDensity_meta[14,3], bulkDensity_meta[15,3],
            bulkDensity_meta[18,3],
            bulkDensity_meta[19,3],
            bulkDensity_meta[25,3]))
...
bulkDensity_2002 <- bulkDensity_2002[, 1:4]

#datasetID <- bulkDensity_meta[5,3]
datasetID <- 'Hungate_incubation'

names(bulkDensity_2001) <- c('depth_cm', 'bulk_density', 'std_error')
bulkDensity_2001$unit <- 'g cm^-3'
bulkDensity_2001$year <- 2001
bulkDensity_2001$CO2_treatment <- 'A'

names(bulkDensity_2002) <- c('plot', 'CO2_treatment', 'depth_cm', 'bulk_density')
bulkDensity_2002$unit <- 'g cm^-3'
bulkDensity_2002$year <- 2002
bulkDensity_2002$depth_cm <- gsub(' cm', '', as.character(bulkDensity_2002$depth_cm))

bulkDensity <- merge(bulkDensity_2002, bulkDensity_2001, all=TRUE)

depth.df <- as.data.frame(t(as.data.frame(strsplit(bulkDensity$depth, '-'))))
names(depth.df) <- c('min', 'max')
row.names(depth.df) <- NULL
#bulkDensity$newdepth <- sprintf('%03d-%03d', as.numeric(as.character(depth.df$min)), as.numeric(as.character(depth.df$max)))
bulkDensity$meanDepth_cm <- (as.numeric(as.character(depth.df$min)) + as.numeric(as.character(depth.df$max)))/2
ggplot(data=bulkDensity) + geom_point(aes(x=meanDepth_cm, y=bulk_density, shape=factor(year), color=CO2_treatment)) + geom_errorbar(aes(x=meanDepth_cm, ymax=bulk_density+std_error, ymin=bulk_density-std_error)) + ylab('Bulk density [g cm^-3]')
```
...
soilProperty$year <- as.numeric(substr(as.character(soilProperty$date), 1,4))
...
ggplot(incubation_2002) + geom_point(aes(x=incubation_day, y=dCO2, color=CO2_treatment), alpha=0.5)
...
ggplot(incubation_2007) + geom_point(aes(x=incubation_day, y=dCO2, color=CO2_treatment), alpha=0.5)
